...
from clusterz.algs import DistributedKZMeans, DistributedKMeans
...
learner = DistributedKZMeans(n_machines=n_machines, n_clusters=n_clusters,
...
# conduct a k-means clustering on the whole data set
central_learner = KMeans(n_clusters=n_clusters)
central_learner.fit(X)
central_cost_with_outliers = -central_learner.score(X)
central_cost_original = -central_learner.score(original_X)

central_learner.fit(original_X)
central_cost_fitted_on_original = -central_learner.score(original_X)

print("\ncentral cost on the original data set (model fitted on the with-outlier data set): {}"
      .format(central_cost_original))
print("central cost with outlier (model fitted on the with-outlier data set): {}"
      .format(central_cost_with_outliers))
print("central cost on the original data set (model fitted on the no-outlier data set): {}"
      .format(central_cost_fitted_on_original))
print("\ncost-original / cost-with-outlier = {0:.3f}"
      .format(central_cost_original / central_cost_with_outliers))
print("cost-original-fitted-no-outlier / cost-original = {0:.3f}"
      .format(central_cost_fitted_on_original / central_cost_original))
