...
class MyTestCase(unittest.TestCase):
    ...
    def test_kmedian_my(self):
        centers = k_median_my(self.X_without_outliers_, n_clusters=2,
                              sample_weights=self.random_weight_without_outliers_)
        assert self.is_centers_match(self.centers_, centers, max_dist=6.001)

    def test_kzmedian(self):
        centers = kz_median(self.X_with_outliers_, n_clusters=2, n_outliers=2,
                            sample_weights=self.random_weight_)
        assert self.is_centers_match(self.centers_, centers, max_dist=6.001)

    def test_DistributedKZMedian(self):
        dkzm = DistributedKZMedian(
            n_clusters=2, n_outliers=2,
            n_machines=2, pre_clustering_routine=KZMedian,
    ...
    def test_BELDistributedKMedian(self):
        bel = BELDistributedKMedian(
            n_clusters=2, n_machines=2,
            pre_clustering_routine=KMedianWrapped, n_pre_clusters=2,
