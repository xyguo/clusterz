...
parameters = {
    'spambase': {'n_machines': [i for i in range(2, 21, 2)],
                 'n_clusters': [i for i in range(10, 101, 10)],
                 'n_outliers': [2 ** i for i in range(5, 10)]
                 },
    'parkinsons': {'n_machines': [i for i in range(2, 21, 2)],
                   'n_clusters': [i for i in range(10, 101, 10)],
                   'n_outliers': [2 ** i for i in range(5, 10)]
                   },
    'pendigits': {'n_machines': [5],#[i for i in range(2, 21, 2)],
                  'n_clusters': [10], #[i for i in range(10, 30, 2)],#[i for i in range(10, 20, 2)],
                  'n_outliers': [2 ** i for i in range(4, 11)]
                },
    'letter': {'n_machines': [i for i in range(2, 21, 2)],
               'n_clusters': [i for i in range(10, 101, 10)],
...
results = {'n_machines_range': np.array(n_machines_range),
           ...
           'seq-greedy-comm-cost': [[] for _ in range(n_repeat)],
           'seq-greedy-cost': [[] for _ in range(n_repeat)],
           'seq-charikar-comm-cost': [[] for _ in range(n_repeat)],
           'seq-charikar-cost': [[] for _ in range(n_repeat)]
           }
...
for r in range(n_repeat):
    ...
    for n_machines, n_clusters, n_outliers in product(n_machines_range,
                                                      ...
                                                      n_outliers_range):
        print("\n" + "==============" * 6)
        print("Repeat {}: n_machines={}, n_clusters={}, n_outliers={}, z_star={}"
              .format(r, n_machines, n_clusters, n_outliers, z_star))
        ...
        seq_baseline_charikar = KZCenter(algorithm='charikar',
                                         n_machines=n_machines, n_clusters=n_clusters,
                                         n_outliers=n_outliers, epsilon=epsilon, debug=False)

        seq_baseline_greedy = KZCenter(algorithm='greedy',
                                       n_machines=n_machines, n_clusters=n_clusters,
                                       n_outliers=n_outliers, epsilon=epsilon, debug=False)
        ...
        seq_models = {'seq-greedy': seq_baseline_greedy,
                      'seq-charikar': seq_baseline_charikar}
...
np.savez("kzm_{}_{}_{}_z_star_{}_dist_vs_seq_exp_results_for_dataset_{}_20170123".
